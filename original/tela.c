...
void tela_inicio(int largura, int altura, char nome[]) {
	...
	if (!al_init())
		cai_fora("problema na inicialização do allegro");
	
	if (!al_install_mouse())
		cai_fora("problema na inicialização do mouse do allegro");
	
	if (!al_init_primitives_addon())
		cai_fora("problema na inicialização de addons do allegro");
	
	al_init_font_addon();
	
	if (!al_init_ttf_addon())
		cai_fora("problema na inicialização do addon de fontes ttf");
...
void tela_atualiza(void) {
	...
	if (tempo_ate_mostrar > 0)	// é muito cedo, dá uma cochilada
	...
	// limpa o canvas em memória, para desenhar a próxima tela
...
void tela_retangulo(float x1, float y1, float x2, float y2, float l, int corl, int corint) {
